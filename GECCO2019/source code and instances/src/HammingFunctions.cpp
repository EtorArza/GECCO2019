...
double HammingFunctions::Newton_raphson_method(double dAvg_val,
...
double HammingFunctions::estimate_theta_mm(int m, int *sigma_0, int **samples)
{
	...
	return Newton_raphson_method((double)dist / m, 0.0, model, -1, NULL);
}
...
int HammingFunctions::which_distance_to_sample_MM(double theta)
{
	...
	return chose_index_given_probabilities(sampling_probabilities, n_);
}
